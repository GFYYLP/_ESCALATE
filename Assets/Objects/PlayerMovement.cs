...
void WrapPosition()
{
    ...
    if (pos.x > worldMax.x) pos.x = worldMin.x;
    else if (pos.x < worldMin.x) pos.x = worldMax.x;

    if (pos.y > worldMax.y) pos.y = worldMin.y;
    else if (pos.y < worldMin.y) pos.y = worldMax.y;
